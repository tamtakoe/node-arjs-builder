...
var postcssUrlMapper      = require('postcss-url-mapper');
...
function compileModule(name, params) {
    ...
    var scriptsStream = series(scriptsSrcStream, scriptsContentStream)
        ...
        .pipe(gulpMark.set('config', name + '/' + params.moduleIndexFile))
    ...
    var base64Config = _.merge({}, base64DefaultConfig, params.inlineImgToCss);

    if (base64Config.excludeFilesStartWith) {
        var excludeFilesStartWith = base64Config.excludeFilesStartWith instanceof Array ? base64Config.excludeFilesStartWith : [base64Config.excludeFilesStartWith];

        base64Config.exclude = base64Config.exclude.concat(excludeFilesStartWith);
    }

    if (base64Config.excludeFilesByRegexp) {
        var excludeFilesByRegexp = base64Config.excludeFilesByRegexp instanceof Array ? base64Config.excludeFilesByRegexp : [base64Config.excludeFilesByRegexp];
        var regexps = excludeFilesByRegexp.map(function (value) {
            return new RegExp(value);
        });

        base64Config.exclude = base64Config.exclude.concat(regexps);
    }

    var joinCssUrl = function(url, type) {
        return path.join(params.cssUrlPrefix || '', url);
    };
    ...
    var stylesStream = gulp.src(params.styles, {cwd: cwd})
        ...
        .pipe(gulpIf(!!params.cssUrlPrefix, gulpPostcss( [postcssUrlMapper(joinCssUrl)] )))
