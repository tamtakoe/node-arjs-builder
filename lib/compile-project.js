...
function compileProject(baseUrl, options) {
    ...
    _.forEach(modules, function(module) {
        _.defaults(module, defaults, options);
    });

    function transform(file, enc, callback) {
        var projectInfo = utils.projectInfoFromPath(file.path, options.baseUrl);
...
function getConcatOptions(name, includeCss) {
    if (includeCss) {
        return [{path: name + '.js',  marks: ['script', 'template', 'config', 'style']}];
    } else {
        return [{path: name + '.js',  marks: ['script', 'template', 'config']}, {path: name + '.css', marks: 'style'}];
    }
}
